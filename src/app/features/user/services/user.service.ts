...
export class UserService {
  ...
  getUserById(id: string, role: string): Observable<serverResponse> {
    const url =
      this.baseUrl + this.userUrl.GET_USER_BY_ID + `?id=${id}&&role=${role}`;
  ...
  //password reset
  forgotPassword(data: object): Observable<serverResponse> {
    const url = this.baseUrl + this.userUrl.FORGOT_PASSWORD;

    return this.httpClient.post<serverResponse>(url, data);
  }

  resetPassword(data: object): Observable<serverResponse> {
    const url = this.baseUrl + this.userUrl.RESET_PASSWORD;
